...
//    weak var delegate:
    ...
    private var startSilence: TimeInterval = 0     // num of seconds of no audio input
    
    func startTimer(isSilent: @escaping ()->Bool) {
        timerStopped = false
        startDate = Date()
        startSilence = startDate!.timeIntervalSince1970
    ...
    nonisolated private func update(isSilent: @escaping ()->Bool) {
        Task { @MainActor in
            ...
            if isSilent() {
                if curSeconds - self.startSilence > 18 {
                    // silent for 30mins, turn off
                    self.timerStopped = true
                }
            } else {
                self.startSilence = curSeconds  // reset silence timer if there is input audio
            }
        }
    }
